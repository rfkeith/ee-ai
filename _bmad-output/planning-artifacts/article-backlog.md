...
|-------|--------------------|---------------------|-------------------|--------|
| Sizing the Team Around the Person, Not the Org Chart | Org-design advice treats team sizing as an org-chart problem (how many ICs per manager). The real question is what the people can carry — a high-performing IC needs a different team shape than a developing one; the right team is one that grows the people in it without overloading them. | _needs author's angle — a moment he sized a team around what one specific person needed to grow (or the inverse: right size on paper, wrong shape for the people in it)_ | An Elegant Puzzle (Larson) — Ch. 2.1 "Sizing teams" (pressure-test required; opening sentences read, full argument not yet verified) | proposed — Leadership gap-filler (department has been empty since Shielding shipped 2026-02-07); Larson's first anchored citation in the corpus |
...
| The Best Architectural Decisions Look Like Over-Engineering Until They Don't | Every architecture decision has a moment where you choose between "what we need right now" and "what we'll need in 18 months." The decision that looks like gold-plating in month one is the one that prevents the migration in month eighteen. Most teams optimise for month one because the cost of being wrong is hidden — but the receipt comes due. | Author's own — **the hot-swap scene** (already logged in [scene-library.md](scene-library.md)): deliberate tech choice, team hot-swapped code in the running cluster via rolling restart, team realised it was foresight not gold-plating. **Counter-example required before drafting:** a time the over-engineered thing didn't pay back (per scene-library flag). | An Elegant Puzzle (Larson) on technical decision-making (chapter verification pending); pairs with Pre-Mortems draft as the *positive* version (pre-mortems prevent failure; this pre-positions for survival) | proposed — scar already on file, counter-example pending. Move the hot-swap entry out of scene-library when this essay is greenlit for outline |
...
| Title | Premise (one line) | Lived stake / angle | Candidate sources | Status |
|-------|--------------------|---------------------|-------------------|--------|
| Stacking vs. Scattering: How to Run Multiple Coding Agents Without Burning Your Attention | Coding agents don't reduce context switching — they relocate it from tasks to sessions, and the new mode is *more* cognitively expensive than task-focus was. Each session demands meta-state tracking ("what's it doing, what does it need, does it need me now"). Four sessions ≠ four times the output. **But the load can be managed:** stacking sessions in the same domain rather than scattering them across unrelated tasks keeps the meta-context coherent. The new skill that distinguishes productive AI-assisted engineers from overwhelmed ones isn't agent quantity — it's session composition. | Author's own — three confirmed angles: (1) **his own multi-agent use** (live during this very planning session — ~5+ parallel concerns including ship/build-infra/refactor/backlog); (2) **observation of his 15 reports** — engineers producing more artifacts while quality of attention drops; (3) **the security-pivot context** where a dropped thread is breach risk, amplifying both upside and risk of inattention. | An Elegant Puzzle (Larson) for possible cognitive-load / attention material (verify); pressure-test against the parked AI Threat draft framing. **Adjacent article in `reference/`: Michael Lawrence, "AI Isn't Replacing Developers. It's Doing Something Worse." (Level Up Coding, 2026-05-11)** — his thesis is *deformation* (skill atrophy across years); yours is *meta-load right now*. Related, not competing; potential cross-cite. | proposed — strongest AI pitch on the board. **Replaces the parked AI Threat Assessment draft** (see Parked). Working title chosen 2026-06-14; subject to revision |
...
| What AI Actually Threatens and What It Doesn't | Superseded by *Stacking vs. Scattering* — the sharper version of the same territory (AI's hidden costs / what's actually changing for engineers). The broad activity-by-activity framing was scar-pending for 4 months and never gained traction; the narrower thesis is more honest and easier to ship. File `src/content/blog/ai/ai-honest-threat-assessment.md` left in repo with `draft: true` for reference. | 2026-06-14 |
...
| An Elegant Puzzle | Will Larson | Sizing the Team Around the Person (proposed); Best Architectural Decisions Look Like Over-Engineering (proposed); possibly Stacking vs. Scattering (proposed) — Larson citations pending chapter verification for all three |
